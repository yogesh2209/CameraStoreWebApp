$(document).ready( function() {
    ...
    $(':submit').on('click', function(e) {
            e.preventDefault();
            document.cookie = "jadrn018" + "= ;expires=-1;path=/";
            $('form').serialize();
            $('form').submit();
        });
    ...
    $('.updateButton').on('click', function() {
        ...
        {
          //Quantity is legal now, have a ajax call and send quantity to server and check if quantity ordered is valid or not
            document.getElementById("emptyCardBox").style.display = "none";
            var cartArray = cart.getCartArray();
            var index = $(this).closest('td').parent()[0].sectionRowIndex-1;
            var sku = cartArray[index][0];
            var params = "sku=" + sku;
            var url = 'http://jadran.sdsu.edu/jadrn018/servlets/servlet/CheckQty?'+params;
            $.ajax({
              url: url,
              type: "get",
              success: function(response){
                getAjaxQtyResponse(response, newQuantity, sku);
              }
          });
        }
    });

    function getAjaxQtyResponse(response, newQuantity, sku){
      var qty_data = 0;
      ...
      for(var i=0; i < tmpArray.length; i++) {
          ...
          innerArray.pop();
          qty_data = innerArray[0];
          }

      if (parseInt(qty_data) > parseInt(newQuantity)) {

        document.getElementById("error_qty").style.display = "none";
        document.getElementById("order_button").disabled = false;
        cart.setQuantity(sku, newQuantity);
        updateDisplay();
        location.reload();
      }
      else if (parseInt(qty_data) == parseInt(newQuantity)) {
        ...
        //Show him error that qty exceeds stock - show him maximum available
        var alertString = "Quantity exceeds the stock. Maximum quantity available is " + qty_data ;
        document.getElementById("error_qty").style.display = "block";
        document.getElementById("error_qty").innerHTML = alertString;
    ...
    function updateDisplay() {
        var cartArray = cart.getCartArray();
            ...
            for(i=0; i < cartArray.length; i++)
            {
                var total_price = 0;
                toWrite += "<tr>";
                toWrite += "<td>"+"<img src=\"http://jadran.sdsu.edu/~jadrn018/proj1/images/"+cartArray[i][0]+"\" alt=\""+cartArray[i][0]+"\""+"width=\"120px\" />"+"</td>";
                ...
                toWrite += "</tr>";
                toWrite += "<tr>";
                toWrite +="<td>"+"<h5>"+'Total Tax (7.75%)'+"</h5>" +"</td>";
                var totalTax = (totalOrderAmount * 0.0775).toFixed(2);
                toWrite += "<td>"+"<h6>"+"$"+totalTax+"</h6>" +"</td>";
                toWrite += "</tr>";
                toWrite += "<tr>";
                toWrite +="<td>"+"<h5>"+'Total Shipping Charges ($5 per item)'+"</h5>" +"</td>";
                var roundedShipping = (cartArray.length * 5).toFixed(2);
